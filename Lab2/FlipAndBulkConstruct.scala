//Flipped construct -- direction of all the signals is reversed i.e. from inputs are changed to outputs and vice versa.
//it is useful where we are using bundles that are input to one module and output to another module.
